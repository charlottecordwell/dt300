...
# Authour: Charlotte
# Name: book_gui.py
# Purpose: Make a gui for a book inventory
...
mlabel = Label(mGui,text='Book Inventory',fg='#509621',bg='white', font =('Comic sans', 20))
mlabel.pack(pady=5)
...
bookNameEntryl = Label(mGui,text='Book Name',fg='#509621',bg='white')
bookNameEntryl.pack(pady =5, anchor=W)
bookNameEntry = Entry(mGui,textvariable=bookName)
bookNameEntry.insert(10,"book name")
bookNameEntry.pack(pady =5, anchor=N)

copiesEntryl = Label(mGui,text='Number of Copies',fg='#509621',bg='white')
copiesEntryl.pack(pady =5, anchor=W)
copiesEntry = Entry(mGui, textvariable=copies)
copiesEntry.insert(10,1)
copiesEntry.pack(pady =5, anchor=N)

#obtain data from a pull down list
genreEntry1 = Label(mGui, text='Genre',fg="#509621", bg="white")
genreEntry1.pack(pady =5, anchor=W)
genreEntry=OptionMenu(mGui, genre, "Science Fiction", "Sport", "Biography", "Political Thriller")
genreEntry.pack(pady =5, anchor=N, ipadx=100)
...
qbutton = Button(mGui,text="DIVISION BY ZERO DETECTED THE UNIVERSE HAS ENDED!",command=Quit, fg='red',bg="black")
qbutton.pack(pady =5)
